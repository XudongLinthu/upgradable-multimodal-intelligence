...
class MMT_VideoQA(nn.Module):
    def __init__(
        ...
        # answer modules
        self.amodel = AModel(out_dim=d_model, sentence_dim=2048, args=args)
    ...
    def forward(
        ...
        if mode == "vqa":
            ...
            elif self.baseline == "to":   
                fusion_proj = self.vqproj(question[:, 0, :])    
            else:
            ...
            fusion_proj = F.normalize(fusion_proj, p=2, dim=len(fusion_proj.shape)-1) * 4.5
            answer_proj = F.normalize(answer_proj, p=2, dim=len(answer_proj.shape)-1) * 4.5
